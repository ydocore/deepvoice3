...
def build_model(training_type='seq2seq'): # linear or world or seq2seq
    # Call the "hparams.builder" function in builder
    model = getattr(builder, hparams.builder)(
        n_speakers=hparams.n_speakers, # 話者数
        speaker_embed_dim=hparams.speaker_embed_dim,
        n_vocab=_frontend.n_vocab, # 文字と音素の数
        embed_dim=hparams.text_embed_dim, # 埋め込み次元
        mel_dim=hparams.num_mels, # メルの分割数
        linear_dim=hparams.fft_size // 2 + 1,
        r=hparams.outputs_per_step, # デコーダで出力するフレーム数
        padding_idx=hparams.padding_idx, 
        dropout=hparams.dropout, # ドロップアウト率
        kernel_size=hparams.kernel_size, # カーネルサイズ
        encoder_channels=hparams.encoder_channels, # エンコーダチャンネル数
        num_encoder_layer=hparams.num_encoder_layer, # エンコーダの層数
        decoder_channels=hparams.decoder_channels, # デコーダのチャンネル数
        num_decoder_layer=hparams.num_decoder_layer, # デコーダの層数
        attention_hidden=hparams.attention_hidden, # アテンションの次元
        converter_channels=hparams.converter_channels, # コンバータのチャンネル数
        num_converter_layer=hparams.num_converter_layer, # コンバータの層数
        query_position_rate=hparams.query_position_rate, # クエリの位置率
        key_position_rate=hparams.key_position_rate, # キーの位置率
        position_weight=hparams.position_weight,
        use_memory_mask=hparams.use_memory_mask,
        trainable_positional_encodings=hparams.trainable_positional_encodings,
        force_monotonic_attention=hparams.force_monotonic_attention, # 単調強制を行うか
